...
const store = {
  ...
  mutations: {
    incrementCartItemCount(state, productData) {
      const item = state.cartList.find((item) => item.id === productData);
      item.count += 1;
    },
  ...
  getters: {
    ...
    getTotalPrice: (state) =>
      state.cartList.reduce((prevSum, productData) => {
        const product = state.ProductList.find(
          (item) => item.id == productData.productId
        );
        return prevSum + product.Price * productData.count;
      }, 0),
